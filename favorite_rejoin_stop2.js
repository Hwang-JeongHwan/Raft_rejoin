...
var orderer2 = '{"id":"orderer2","state":"follower","term":2} '; //JSON형식으로 데이터 저장 
...
client.on('message', (msg, rinfo) => {
 ...
 /* if(i.rejoin_state == 'rejoin'){
  ...
  if(orderer_parse.state == "favorite1" && orderer_parse.rejoin == 'yes' &&i.copy!='ok' &&i.copy !='finish' && i.try == '1'){ 
    ...
    "favorite1port":9002,"rejoincopy":"send"}`;
  ...
  if(orderer_parse.state=='favorite1'&&i.copy == 'ok' &&i.finish != 'finish' && orderer_parse.rejoin =='yes'){
    if (orderer_parse.stop == true ){
      ...
      "favorite1port":9002,"rejoincopy":"send","retry":"yes"}`;
  ...
  if(orderer_parse.state=='favorite1'&&i.finish == 'finish' && orderer_parse.rejoin =='yes'){
    ...
    var filesize = getFilesizeInBytes('./ledger2.txt');
 ...
 // console.log('cmtime :',cmtime);
  ...
  if((i.candidate =='orderer5'||i.candidate =='orderer1'||i.candidate =='orderer3'||i.candidate =='orderer4')
