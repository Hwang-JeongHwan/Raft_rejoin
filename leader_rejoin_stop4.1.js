...
//   }  
  if(orderer_parse.state == 'leader' &&i.rejoin == 'yes'){
    orderer_parse.rejoin = 'yes';//오더러의 rejoin이 yes면 장부를 복사해주어야함 
 ...
 /* if(i.rejoin_state == 'rejoin'){
  ...
  if(orderer_parse.state=='leader'&&i.copy == 'ok' &&i.finish != 'finish' && orderer_parse.rejoin =='yes'){
    if (orderer_parse.stop == true ){
      ...
      "leaderport":9004,"rejoincopy":"send","retry":"yes"}`;
  ...
  if (i.id==='app' && i.finish !='finish' && i.state != 'dead' && i.state != 'rejoin'){ 
    ...
    if(before_cnt != i.cnt){  //이전 카운트와 현재 들어온 메시지의 카운트가 다를경우만 장부에 저장
      ...
      if(orderer_parse.state =='dead'){ //애플리케이션의 값을 5개 받으면 
        ...
        if(orderer_parse.state !='dead' &&orderer_parse.state!='rejoin' && i.rejoinordererid != "orderer5"){
    ...
    if(orderer_parse.state == 'leader' && i.orderer_state =='ok'&& i.orderer_id != before_id ){//ok메시지가 오고 orderer가 leader 면
